% Tesis ITAM CLASS -- version 0.1 (13 - Abr - 2015)
% Clase para las tesis del ITAM
%
% 13 - Abr - 2015 	Victor Martinez 	victor.martinez (at) itam.mx
% LICENSE: Creative Commons SA-BY 3.0
%
%
% Este documento presenta un ejemplo de uso de la plantilla
% El estudiante es libre de modificar este archivo a su gusto
%
\documentclass{docITAM}
\usepackage[utf8]{inputenc}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage[backend=biber, maxbibnames=99]{biblatex}
\usepackage[paperwidth=17cm, paperheight=22.5cm, bottom=2cm, left=2.5cm, right=2cm, headheight=13.08pt]{geometry}
\usepackage{numprint}
\usepackage{csquotes}
\usepackage{float}
\usepackage{multirow}
\usepackage{hyperref}
\usepackage{subfig} % For multiple images in a single image
\usepackage[font={footnotesize}, labelfont=bf, textfont=it]{caption}
\usepackage[toc,page]{appendix}
% To customize captions in floating environments (i.e., images)
% Available sizes: scriptsize, footnotesize, small, normalsize, large, Large

\addbibresource{bibliography.bib} %Imports bibliography file
\bibliography{bibliography.bib}

\title{Una comparación de métodos frecuentistas y aproximaciones Bayesianas en la implementación de redes neuronales convolucionales en un contexto de aprendizaje activo}
% English:
% A comparison of frequentist methods and Bayesian approximations in the implementation of Convolutional Neural Networks in an Active Learning setting

% Old:
% \title{Una comparación de métodos Bayesianos y frecuentistas en la implementación de redes neuronales convolucionales en un contexto de aprendizaje activo}
% % A comparison of Bayesian and frequentist methods implementing Convolutional Neural Networks in an Active Learning setting




\author{Mario Humberto Becerra Contreras}
\degree{MAESTRO EN CIENCIAS EN COMPUTACIÓN}
\advisor{Dr. Alfredo Garbuno Iñigo}
\year{2019}

\begin{document}
	\npthousandsep{,}
	\pagenumbering{gobble}
	\maketitle
	\publicationrights


	% \include{dedication}
	% \include{acknowledgements}

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% ABSTRACT
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\include{abstract}

	\selectlanguage{english}
	\setcounter{page}{1}
	\pagenumbering{roman}

	\tableofcontents
	\listoffigures
	%\listoftables
	\newpage

	\pagenumbering{arabic}
	\setcounter{page}{1}

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% CONTENT
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\include{Chapters/introduction}
	\include{Chapters/machine_learning}
	\include{Chapters/variational_inference}
	\include{Chapters/ANN}
	\include{Chapters/active_learning}
	\include{Chapters/results}
	\include{Chapters/conclusions}
	\include{Chapters/appendix}




	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% APPENDIX
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\appendix
	% \include{Chapters/appendixA}

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% BIBLIOGRAPHY
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\clearpage
	\addcontentsline{toc}{chapter}{References}



\begingroup
\raggedright
\sloppy
\printbibliography
\endgroup

% \nocite{*}

\end{document}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End:
