...
\end{equation}
where $\mathcal{F}$ is the family of functions of the form $f(X) = X\boldsymbol{\theta}$ with $\boldsymbol{\theta} \in \mathbb{R}^p$. Thus, the  estimate $\hat{f}$ is $\hat{f}(X) = X \hat{\boldsymbol{\theta}}$, where $\hat{\boldsymbol{\theta}}$ minimizes the error function.

The linear regression model belongs to a type of methods called \textbf{parametric methods}, in which the modeler first makes an assumption about the functional form of $f$, and then trains the model by choosing the parameters that minimize a previously selected loss function \cite[p.~21]{james2013introduction}. In the case of linear regression, the form of $f$ is assumed to be $f(X) = X\boldsymbol{\theta}$, and the parameters chosen are the elements of the $\hat{\boldsymbol{\theta}}$ vector that minimize the squared error function of the model.
...
In the case of linear regression, it is commonly assumed that the error term in \eqref{eq:general_learning_model} has a Gaussian distribution such that $\varepsilon_i \sim \normaldist{0}{\sigma^2}$, then $y_i \sim \normaldist{x_i^T \boldsymbol{\theta}}{\sigma^2}$ for $i \in \left\{ 1, \ldots, n \right\}$. It is also usually assumed that the observations $y_i$ come from a random sample and are independent of each other. Therefore, the log-probability of the sample is
\begin{equation}
  \label{eq:gaussian_likelihood}
  %L(\boldsymbol{\theta}) = \sum_{i = 1}^n \log \frac{1}{\sqrt{2 \pi \sigma^2}} e^{-\frac{(y_i - x_i^T \boldsymbol{\theta})^2}{2\sigma^2}},
  L(\boldsymbol{\theta}) = \sum_{i = 1}^n \log \left[ \frac{1}{\sqrt{2 \pi \sigma^2}} e^{- \frac{(y_i - x_i^T \boldsymbol{\theta})^2}{2\sigma^2}} \right],
\end{equation}
...
Another approach to the learning problem is the \textbf{Bayesian approach}, in which a prior distribution on the parameters $\boldsymbol{\theta}$ is assumed, and then the knowledge about them is updated with data. That is, the modeler first quantifies the knowledge that they may have about $\boldsymbol{\theta}$ using a prior distribution $\prob{\boldsymbol{\theta}}$ and then computes the posterior distribution of $\boldsymbol{\theta}$ given $X$ and $y$ using Bayes' theorem as such
\begin{equation}
  \label{eq:bayes_theorem}
  \prob{\boldsymbol{\theta} | y, X } = \frac{\prob{y | \boldsymbol{\theta}, X} \prob{\boldsymbol{\theta} | X}}{\prob{y | X}} = \frac{\prob{y | \boldsymbol{\theta}, X} \prob{\boldsymbol{\theta} | X}}{\int \prob{y | \boldsymbol{\theta}, X} \prob{\boldsymbol{\theta} | X} d\boldsymbol{\theta}}.
\end{equation}
...
% The multiplier $\prob{y | \boldsymbol{\theta}, X}$ is called the likelihood, and is the probability of the data given the parameters.
...
\begin{equation}
  \label{eq:bayes_theorem_prop}
    \prob{\boldsymbol{\theta} | y, X} \propto \prob{y | \boldsymbol{\theta}, X} \prob{\boldsymbol{\theta} | X}.
\end{equation}
...
\begin{equation}
  ...
  \prob{y^* | X, y, x^*} = \int \prob{y^* | \boldsymbol{\theta}, x^*} \prob{\boldsymbol{\theta} | X, y} d\boldsymbol{\theta}.
\end{equation}
...
\begin{equation}
  \prob{y_i = 1 | x_i, \boldsymbol{\theta}} = \sigma(\theta_0 + \theta_1 x_i^{(1)} + \theta_2 x_i^{(2)}) = \frac{1}{1 + e^{-\left( \theta_0 + \theta_1 x_i^{(1)} + \theta_2 x_i^{(2)} \right)}},
\end{equation}
with $x_i = \left[ x_i^{(1)}, x_i^{(2)} \right]^T$. Thus
\begin{equation}
  \prob{y_i = 0 | x_i, \boldsymbol{\theta}} = 1 - \prob{y_i = 1 | x_i} = \frac{1}{1 + e^{\theta_0 + \theta_1 x_i^{(1)} + \theta_2 x_i^{(2)}}}.
\end{equation}

This means that $y_i$ is fully described as a Bernoulli distribution with parameter $p_i =\prob{y_i = 1 | x_i, \boldsymbol{\theta}}$. When the response variable is Bernoulli distributed, the likelihood of $n$ independent observations is
\begin{equation}
...
\begin{equation}
  \prob{X | \boldsymbol{\theta}} = \prod_{i = 1}^n  p_i(\boldsymbol{\theta})^{y_i}\left(1 - p_i(\boldsymbol{\theta}) \right)^{1 - y_i},
\end{equation}
...
\begin{equation}
  \prob{\boldsymbol{\theta} | y, X} \propto
  \left[ \prod_{i = 1}^n  p_i(\boldsymbol{\theta})^{y_i}\left(1 - p_i(\boldsymbol{\theta}) \right)^{1 - y_i} \right]
  \left[ \prod_{k = 0}^2 \frac{1}{\sqrt{2 \pi \sigma^2}} e^{- \frac{(\theta_k - \mu_k)^2}{2\sigma^2}} \right].
  %\left[ \prod_{k = 0}^2 \sqrt{\frac{\tau}{2 \pi}} e^{- \tau \frac{\left( \theta_k - \mu_k \right)^2}{2}} \right].
\end{equation}
...
\begin{equation}
  \begin{split}
    L(\boldsymbol{\theta}) & = - \sum_{i=1}^{n} \left[ y_i \log(\sigma(\boldsymbol{\theta}^T x_i)) + (1-y_i) \log(1-\sigma(\boldsymbol{\theta}^T x_i)) \right] \\
    & = - \sum_{i=1}^{n}{\ell_i(\boldsymbol{\theta})}
  \end{split}
\end{equation}
where $\sigma(\cdot)$ is the logistic sigmoid function, $\boldsymbol{\theta}^T x_i = \sum_{j=0}^{p}{\theta_j x_{ij}}$, $x_{i1} = 1$ for all $i \in \left\{1, ..., n \right\}$ and $\ell_i(\boldsymbol{\theta})$ is defined as
\begin{equation}
  \ell_i(\boldsymbol{\theta}) = y_i \log(\sigma(\boldsymbol{\theta}^T x_i)) + (1-y_i) \log(1-\sigma(\boldsymbol{\theta}^T x_i)).
\end{equation}
...
\begin{equation}
  \begin{split}
    ...
    \frac{y_i \sigma'(\boldsymbol{\theta}^T x_i) x_{ij} }  {\sigma(\boldsymbol{\theta}^T x_i)} + \frac{(1 - y_i) (-1) \sigma'(\boldsymbol{\theta}^T x_i) x_{ij}} {1 - \sigma(\boldsymbol{\theta}^T x_i)} \\
    & = \frac{\sigma'(\boldsymbol{\theta}^T x_i) x_{ij} y_i}{\sigma(\boldsymbol{\theta}^T x_i)} - \frac{(1 - y_i) \sigma'(\boldsymbol{\theta}^T x_i) x_{ij}}{1 - \sigma((\boldsymbol{\theta}^T x_i))} \\
    & = \sigma'(\boldsymbol{\theta}^T x_i) x_{ij} \left(\frac{y_i}{\sigma(\boldsymbol{\theta}^T x_i)} - \frac{1-y_i}{1-\sigma(\boldsymbol{\theta}^T x_i)} \right) \\
    & = \sigma'(\boldsymbol{\theta}^T x_i) x_{ij} \left(\frac{y_i - y_i \sigma(\boldsymbol{\theta}^T x_i) -
    \sigma(\boldsymbol{\theta}^T x_i) + y_i \sigma(\boldsymbol{\theta}^T x_i)}{\sigma(\boldsymbol{\theta}^T x_i)(1-\sigma(\boldsymbol{\theta}^T x_i))} \right) \\
    & = x_{ij}(y_i - \sigma(\boldsymbol{\theta}^T x_i)).
  \end{split}
\end{equation}
