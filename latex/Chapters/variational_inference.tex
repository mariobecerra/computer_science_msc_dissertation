...
This behavior can be appreciated in figure \ref{fig:KL_example}, which shows forward and reverse KL divergence of a mixture of Gaussians and two unimodal Gaussian distributions. The blue distribution in the figure is the mixture of Gaussians, and the red distributions are the unimodal Gaussians. On the left, the zero-avoiding behavior is evident because it avoids the areas in which the mixture takes zero values, and because it has a lower forward KL divergence; whereas on the right the zero-forcing behavior can be appreciated because the red distribution has a mode where the blue distribution has zero values, and it has got a lower reverse KL divergence.

\begin{figure}[H]
  \centering
  \subfloat[$\kl{p}{q} < \kl{q}{p}$]{\includegraphics[width=0.5\textwidth]{KL_example_1.pdf}}
  \hfill
  \subfloat[$\kl{p}{q} > \kl{q}{p}$]{\includegraphics[width=0.5\textwidth]{KL_example_2.pdf}}
