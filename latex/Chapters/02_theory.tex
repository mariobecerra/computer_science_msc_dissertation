...
%The posterior predictive distribution for a new observation pair $(x^*, y^*)$ can be approximated using the variational approximation. In particular, we could take $T$ samples from the variational  as such
%
%\begin{equation*}
  %\begin{split}
      %p(y^* | X, y, x^*) &=
        %\int \prob{y^* | \theta, x^*} \prob{\theta | X, y} d\theta \\
      %& \approx \int \prob{y^* | \theta, x^*} \q{\theta | X, y} d\theta \\
      %& \approx \frac{1}{T} \sum_{t = 1}^T \prob{y^* | \theta, x^*},
  %\end{split}
%\end{equation*}
