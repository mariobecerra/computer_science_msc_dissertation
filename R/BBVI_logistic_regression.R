...
create_data = function(N, real_theta, seed = 0){
  set.seed(seed)
  # P = length(real_theta)
  # X = mvrnorm(N, mu = rep(0, P), Sigma = diag(rep(1, P)))
  # y = rbinom(N, 1, sigmoid(dot(X, real_theta)))
  x = rnorm(N)
  pr = sigmoid(real_theta[1] + real_theta[2]*x)
  y = rbinom(N, 1, pr)
  return(list(X = cbind(ones(N), x), y = y))
}
...
vi = function(X, y, prior_sigma = 10000, max_iter = 6000, S = 10, eta = 1.0, seed = 0){
  ...
  for(t in 1:max_iter){
    ...
    stop_criterion = norm_delta_lambda_t
    if(t %% 100 == 1){
      cat("", "\n")
      cat("Iteration: ", t, "\n")
      cat("Mu: ", mu, "\n")
...
dat = create_data(500, real_theta, seed = 0)

mod_glm <- glm(dat$y ~ dat$X[,2], family = "binomial")

mod = vi(dat$X, dat$y, S = 50, max_iter = max_iter, seed = 0)
...
post_dist_plot = tibble(x = c(mu + 4*sigma, mu - 4*sigma)) %>% 
  ggplot(aes(x)) + 
  geom_vline(xintercept = real_theta, col = "grey", linetype = "dashed") +
  geom_vline(xintercept = mod_glm$coefficients, col = colours, linetype = "dotted") +
...
mus_iteration_individual_plot = mod$mus %>% 
  ...
  ggplot() +
  # geom_hline(yintercept = real_theta[1], linetype = "dotted", color = colours[1], size = 0.4) +
  # geom_hline(yintercept = real_theta[2], linetype = "dotted", color = colours[2], size = 0.4) +
  geom_hline(yintercept = mod_glm$coefficients[1], linetype = "dotted", color = colours[1], size = 0.4) +
  geom_hline(yintercept = mod_glm$coefficients[2], linetype = "dotted", color = colours[2], size = 0.4) +
...
mus_iteration_joint_plot = mod$mus %>% 
  ...
  geom_point(data = tibble(V1 = mod_glm$coefficients[1], V2 = mod_glm$coefficients[2]),
             aes(V1, V2),
             colour = "red", size = 1) +
  # geom_point(data = tibble(V1 = real_theta[1], V2 = real_theta[2]),
  #            aes(V1, V2),
  #            colour = "red", size = 1) +
...
# test_dat = create_data(20, real_theta, seed = 0)
# 
